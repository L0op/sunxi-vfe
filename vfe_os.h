...
#define vfe_dbg(l,x,arg...) if(vfe_dbg_en && l <= vfe_dbg_lv) printk(KERN_EMERG "[VFE_DEBUG]"x,##arg)
//print when error happens
#define vfe_err(x,arg...) printk(KERN_ERR "[VFE_ERR]"x,##arg)
#define vfe_warn(x,arg...) printk(KERN_EMERG "[VFE_WARN]"x,##arg)
//print unconditional, for important info
#define vfe_print(x,arg...) printk(KERN_EMERG "[VFE]"x,##arg)
