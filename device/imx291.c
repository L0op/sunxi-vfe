/*
 *
 * Copyright (c) 2018 <haoxiansen@zhitongits.com.cn>.
 *
...
MODULE_AUTHOR("haoxiansen@zhitongits.com.cn");
...
#define DEV_DBG_EN      1
#if(DEV_DBG_EN == 1)
#define vfe_dev_dbg(x,arg...) printk(KERN_EMERG "[IMX291]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...)
#endif
#define vfe_dev_err(x,arg...) printk(KERN_EMERG "[IMX291]"x,##arg)
#define vfe_dev_print(x,arg...) printk(KERN_EMERG "[IMX291]"x,##arg)

#define LOG_ERR_RET(x)  { \
                          int ret;  \
                          ret = x; \
                          if(ret < 0) {\
                            vfe_dev_err("error at %s(ret:%d)\n",__func__, ret);  \
                            return ret; \
                          } \
                        }
...
{
  unsigned char rdval[2] = {};
  printk(KERN_EMERG "----------------imx291 id(0xa0b2) is ");
  LOG_ERR_RET(sensor_read(sd, 0x3008, &rdval[0]));
  LOG_ERR_RET(sensor_read(sd, 0x301e, &rdval[1]));
...
static struct sensor_format_struct {
  ...
  int bpp;   /* Bytes per pixel */
	...
	{
		.desc	   = "Raw RGB Bayer",
		//.mbus_code = V4L2_MBUS_FMT_SRGGB12_12X1,	//MIPI
		.mbus_code = V4L2_MBUS_FMT_SBGGR10_1X10,	//10b raw
		.regs 	   = sensor_fmt_raw,
